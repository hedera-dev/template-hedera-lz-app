...
const STRATEGY_ABI = [
    ...
    'function invest(uint256,uint256) returns (uint256,uint256)',
]
const ERC20_ABI = ['function approve(address,uint256) returns (bool)']

task('lz:strategy:invest', 'Swap asset into HBAR + HUSTLERS using the Hedera ETF strategy')
    ...
    .setAction(async (args, hre) => {
        ...
        const tx = await strategy.invest(amountIn, deadline)
        await tx.wait()
    })
