...
import { CheckIcon, ClipboardDocumentIcon, KeyIcon, ShieldExclamationIcon, XMarkIcon } from "@heroicons/react/24/outline";
import { useCopyToClipboard } from "~~/hooks/scaffold-hbar";
import { notifyError, notifySuccess } from "~~/utils/scaffold-hbar/notification";
...
export const RevealBurnerPKModal = () => {
  ...
  const handleCopyPK = async () => {
    try {
      ...
      notifySuccess("Burner wallet private key copied to clipboard");
    } catch (e) {
      notifyError(e instanceof Error ? e.message : "Failed to copy burner wallet private key");
      ...
      <label htmlFor="reveal-burner-pk-modal" className="modal cursor-pointer">
        <label className="modal-box relative bg-base-100 border border-base-300 rounded-2xl shadow-xl p-6 max-w-md">
          ...
          <p className="text-sm text-base-content/70 mb-5 m-0">
            Your private key grants <span className="font-semibold text-base-content">full access</span> to this wallet. It
            is stored <span className="font-semibold text-base-content">temporarily</span> in your browser and will be lost
