...
export const SetBurnerPKModal = () => {
  ...
  const handleSavePK = () => {
    try {
      ...
      notifySuccess("Burner wallet private key updated. Wallet will be disconnected.");
      ...
      notifyError(e instanceof Error ? e.message : "Failed to set burner wallet private key");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div>
      ...
      <label htmlFor="set-burner-pk-modal" className="modal cursor-pointer">
        <label className="modal-box relative bg-base-100 border border-base-300 rounded-2xl shadow-xl p-6 max-w-md">
          ...
          <button className="btn btn-primary w-full gap-2" onClick={handleSavePK} disabled={isSaving || !pkInput.trim()}>
