...
// ---------------------------------------------------------------------------
// Contracts: define each OApp that participates in OVault messaging.
// - Asset OFT lives on both chains (Base + Hedera)
// - Share side uses ShareOFT on Base and ShareOFTAdapter on Hedera (hub)
// ---------------------------------------------------------------------------
...
// Gas options used for all pathways in this example. Tune for production.
...
// ---------------------------------------------------------------------------
// Simple Workers metadata extension
// - Adds SimpleDVNMock + SimpleExecutorMock as recognized DVN/Executor names
// - Uses deployments from ./deployments/<network>/
// ---------------------------------------------------------------------------
const customFetchMetadata = async (): Promise<IMetadata> => {
    const defaultMetadata = await defaultFetchMetadata()

    const configuredContracts = [hederaAssetOft, baseAssetOft, hederaShareAdapter, baseShareOft]
...
// ---------------------------------------------------------------------------
// Pathways
// 1) Asset OFT mesh: Hedera <-> Base
// 2) Share mesh: Hedera ShareOFTAdapter <-> Base ShareOFT
// ---------------------------------------------------------------------------
...
export default async function () {
    ...
    return {
        contracts: [
            { contract: hederaAssetOft },
            { contract: baseAssetOft },
            { contract: hederaShareAdapter },
